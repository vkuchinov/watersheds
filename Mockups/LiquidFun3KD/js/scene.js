...
var d3Renderer = {
    ...
    calculatePositions: function(system){
        
        var output = [];
        
        var length = system.GetParticleCount();
        ...
        for(var i = 0; i < length; i++){
            output.push({x: positionBuf[i * 2], y: positionBuf[i * 2 + 1]});
        }
	...
	drawParicles: function(selection, system){ 
        ...
        var positions = d3Renderer.calculatePositions(system);
		...
		particleGroup.each(function(pg){
			...
			dataSet
            .attr("fill",  function(d, i){ if(nodes[i].getClosestDistance(nodes[i].id, positions) > 0.07) { return d3.hsl(nodes[i].hue, 0.0, 1.0); } else { return d3.hsl(nodes[i].hue, 0.8, 0.7); } })
            ...
            .attr("x1", function(d, i){ return positionBuf[(i + offset) * 2]; })
            .attr("y1", function(d, i){ return positionBuf[(i + offset) * 2 + 1]; })
