...
THEGAME.Engine = function (babylonEngine) {
    ...
    THEGAME.Engine.prototype.createScene = function () {
        ...
        that._grassMaterial.diffuseColor = new BABYLON.Color3(1.0, 1.0, 1.0);
        ...
        vertices.push(new b2Vec2(-8, -6));
        vertices.push(new b2Vec2(8, -6));
        vertices.push(new b2Vec2(8, -4));
        vertices.push(new b2Vec2(-8, -4));
        ...
        vertices.push(new b2Vec2(-8, 4));
        vertices.push(new b2Vec2(-7.5, 4));
        vertices.push(new b2Vec2(-7.5, -4));
        vertices.push(new b2Vec2(-8, -4));
...
//        particleSystem.CreateParticleGroup(pgd);
    ...
    THEGAME.Engine.prototype.createBabylonParticles = function (particleSystem, color) {
        ...
        particleSystem.minSize = that._particleRadius * 1.5;
        particleSystem.maxSize = that._particleRadius * 1.5;
    ...
    THEGAME.Engine.prototype.particleUpdateFunction = function (particlesBJS) {
        ...
        for (var i = 0, max = world.particleSystems.length; i < max; i++) {
            ...
            for (var i = 0; i < maxParticles && i < particlesBJS.length; i += 2) {
                points.push({x: particles[i], y: particles[i + 1]});
            }
            ...
            for (var i = 0, c = 0; i < maxParticles && i < particlesBJS.length; i += 2, c += 4) {
                ...
                var nearest = tree.nearest({x: particlesBJS[i].position.x, y: particlesBJS[i].position.y}, 7);
                var distances = [];
                
                for(var j = 0; j < nearest.length; j++){
                    
                    distances.push(sqrtDistance({x: particlesBJS[i].position.x, y: particlesBJS[i].position.y}, {x: nearest[j][0].x, y: nearest[j][0].y }));
                    
                }
//                
                //if(i < 4) { console.log(Math.max(Math.max.apply(null, distances))); }
                
                
                if(Math.max.apply(null, distances) < 0.16) {  
                ...
                particlesBJS[i].color.r = 1.0;
                particlesBJS[i].color.g = 1.0;
                particlesBJS[i].color.b = 1.0;
                ...
                switch (fixture.shape.type) {
                    case 0:
                        ...
                        if (fixture.babylonShape === undefined) {
                            ...
                            var shape = BABYLON.Mesh.CreateCylinder("cylinder", 0.0, fixture.shape.radius * 2, fixture.shape.radius * 2, 24, 1, that._scene, false);
