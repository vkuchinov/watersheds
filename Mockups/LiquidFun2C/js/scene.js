...
function Node(id_, parent_){
    ...
    this.findClosestNeighbour = function(index_, particles_){
        
        var tmpClosest = Number.POSITIVE_INFINITY;
        //closet by now is 0.1
        
        var positions = particles_.GetPositionBuffer();
        var x = positions[index_ * 2];
        var y = positions[index_ * 2 + 1];
        
        for(var i = 0; i < particles_.GetParticleCount(); i++){
            
            if(i != index_){
                
                var dist = Math.sqrt(Math.pow(positions[i * 2] - positions[index_ * 2], 2.0) + Math.pow(positions[i * 2 + 1] - positions[index_ * 2 + 1], 2.0));
                if(dist < tmpClosest) { tmpClosest = dist; }
           
            }
            
        }
        
        this.closestDistance = tmpClosest;
...
var d3Renderer = {
	...
	render: function(world){
        ...
        d3Renderer.updateClosestDistances(world.particleSystems[0]);
    ...
    updateClosestDistances: function (particles_){
        
        var points0 = [];
        var points = [];
        //var nearest;
    
        var buffer = particles_.GetPositionBuffer();
        
        for(var i = 0; i < particles_.GetParticleCount(); i++){ 
            
            var xx, yy;
            //if(buffer[i * 2] != undefined) { xx = buffer[i * 2]; } else { xx = 0.0; }
            //if(buffer[i * 2 + 1] != undefined) { yy = buffer[i * 2 + 1]; } else { yy = 0.0; }
                  
             xx = buffer[i * 2];
             yy = buffer[i * 2 + 1];
            
            points.push({x: xx, y: yy});
            points0.push({x: xx, y: yy});
        }
        var distance = function(a, b){ return Math.pow(a.x - b.x, 2) +  Math.pow(a.y - b.y, 2);}
        var tree = new kdTree(points, distance, ["x", "y"]);
        //var nearest = tree.nearest(points[0], 2); console.log(nearest);
        for(var i = 0; i < nodes.length; i++){ var nearest = tree.nearest({ x: points0[nodes[i].id].x, y: points0[nodes[i].id].y}, 2); 
        nodes[i].closestDistance = Math.sqrt(Math.pow(nearest[0][0].x - points0[nodes[i].id].x, 2) + Math.pow(nearest[0][0].y - points0[nodes[i].id].y, 2));
        if(i < 1) { console.log(nodes[i].closestDistance); }
        }
        
        //if(debug) {  console.log(points.length + " " + particles_.GetParticleCount()); console.log(points); debug = false; }
    },
	...
	drawParicles: function(selection, system){ 
		...
		particleGroup.each(function(pg){
            ...
            .attr("fill",  function(d, i){ 
                ...
                if(nodes[i].closestDistance < 0.06){
                return d3.hsl(nodes[i].hue, 0.8, 0.7); 
                }
                else {
                return d3.hsl(nodes[i].hue, 0.0, 1.0);   
                }
