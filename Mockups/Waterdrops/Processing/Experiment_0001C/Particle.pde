class Particle extends PVector {
  ...
  void draw(PGraphics scene_) {

    scene_.colorMode(HSB);
    scene_.noStroke();
    if(c2 == -1) { scene_.fill(32); } else { scene_.fill(c2, 240, 240); }
    scene_.ellipseMode(CENTER);
    float r = map(depth, -50, 50, 0, 16.0);
    scene_.ellipse(x, y, r, r);
  }
}
