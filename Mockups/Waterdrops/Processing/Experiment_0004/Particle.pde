    ...
    for (int r = 0; r < ripples.size (); r++) {
      ...
      if (PVector.dist(new PVector(x, y), new PVector(aX, aY)) < 16) { 
        ...
        if(direction(x, y, aX, aY) == 1){
          ...
          force.x = velocity.x * map(PVector.dist(new PVector(x, y), new PVector(aX, aY)), 0, 16, 1.6, 0);
          force.y = velocity.y * map(PVector.dist(new PVector(x, y), new PVector(aX, aY)), 0, 16, 1.6, 0);
