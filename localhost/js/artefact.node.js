function Node(id_, xml_, cx_, cy_) {
    ...
    this.lerp = function (value0_, value1_, t_) {
        return (1.0 - t_) * value0_ + t_ * value1_;
    }
    ...
    this.calculateColor = function (value_) {
        ...
        if (value_ == 1) {
            ...
            var result = {
                r: parseInt(this.lerp(this.normalRGB.r, this.foamRGB.r, value_)),
...
function Vector2(x_, y_) {
    ...
    this.dist = function (v_) {
        return Math.sqrt(Math.pow(v_.x - this.x, 2.0) + Math.pow(v_.y - this.y, 2.0));
    }
    this.dist2 = function (v0_, v1_) {
        return Math.sqrt(Math.pow((v1_.x - v0_.x), 2.0) + Math.pow((v1_.y - v0_.y), 2.0));
    }
