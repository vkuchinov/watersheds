    ...
    for (int r = 0; r < ripples.size (); r++) {
      ...
      if (PVector.dist(new PVector(x, y), new PVector(aX, aY)) < 16) { 
        if (direction(x, y, aX, aY) == 1 ) {  
          stroke(0, 255, 255);
          depth +=  PVector.dist(new PVector(x, y), new PVector(aX, aY)) * 1.5; add = new PVector(v.x, v.y); add.mult(PVector.dist(new PVector(x, y), new PVector(aX, aY)) * 0.1);
        } else { 
          depth -=  1 / PVector.dist(new PVector(x, y), new PVector(aX, aY)) * 1.5; add = new PVector(-v.x, -v.y); add.mult(PVector.dist(new PVector(x, y), new PVector(aX, aY)) * 0.1);
        } 
        { 
          stroke(255, 0, 255);
          line(x, y, aX, aY);
